...
from app.line_client import LineSendError, push_text_message
...
def send_conversation_message(
    ...
    if conversation.channel == Channel.whatsapp:
    ...
    elif conversation.channel == Channel.line:
        to_user = conversation.external_thread_id
        _, _, line_token = resolve_line_credentials(user.tenant)
        if not line_token:
            raise HTTPException(status_code=400, detail="LINE channel access token not configured")
        try:
            result = push_text_message(
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Send is not supported for channel {conversation.channel}",
        )
